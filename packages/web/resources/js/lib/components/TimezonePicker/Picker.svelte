<script>
  ...
  import { scrollIntoView, uid, slugify, keyCodes, ungroup, filter, pick } from './utils';
  ...
  $: filteredZones = userSearch && userSearch.length > 0 ? filter(userSearch, availableZones) : validZones.slice();
...
<button class="px-3 py-1.5 justify-between block w-full rounded-md border-0 bg-accents-200 text-accents-900 shadow shadow-accents-200-sm ring-1 ring-inset ring-accents-300 placeholder:text-accents-500 focus:ring-2 focus:ring-inset focus:ring-main-500 sm:text-sm sm:leading-6" bind:this={toggleButtonRef} type="button" aria-label={`${currentZone[0]} is currently selected. Change timezone`} aria-haspopup="listbox" data-toggle="true" aria-expanded={expanded} on:click={toggleExpanded} on:keydown={toggleExpanded}>
  <div>{currentZone[0]}</div>
  <div class="flex">
    <span class="text-xs px-2">GMT {currentZone[1]}</span>
...
{#if expanded}
  <div class="tz-dropdown" transition:slide on:introend={scrollToHighlighted} on:keydown={keyDown}>
    <span class="sr-only" id={labelId}> Select a timezone from the list. Start typing to filter or use the arrow keys to navigate the list </span>
    <div class="input-group">
      <!-- svelte-ignore a11y-autofocus -->
      <input class="block w-full rounded-md border-0 py-1.5 bg-accents-200 text-accents-900 shadow shadow-accents-200-sm ring-1 ring-inset ring-accents-300 placeholder:text-accents-500 focus:ring-2 focus:ring-inset focus:ring-main-500 sm:text-sm sm:leading-6" id={searchInputId} bind:this={searchInputRef} type="search" aria-autocomplete="list" aria-controls={listBoxId} aria-labelledby={labelId} autocomplete="off" autocorrect="off" placeholder="Search..." bind:value={userSearch} autofocus />

      {#if userSearch && userSearch.length > 0}
        <button bind:this={clearButtonRef} title="Clear search text" on:click={clearSearch}> &times; </button>
      {/if}
    </div>

    <ul tabindex="-1" class="tz-groups" id={listBoxId} role="listbox" bind:this={listBoxRef} aria-labelledby={labelId} aria-activedescendant={currentZone && `tz-${slugify(currentZone[0])}`}>
        ...
        {#if groupHasVisibleChildren(group, filteredZones)}
          ...
          {#each Object.entries(groupedZones[group]) as [zoneLabel, zoneDetails]}
            {#if filteredZones.includes(zoneLabel)}
              <li role="option" tabindex="0" id={`tz-${slugify(zoneLabel)}`} bind:this={listBoxOptionRefs[zoneLabel]} aria-label={`Select ${zoneDetails[0]}`} aria-selected={highlightedZone === zoneDetails[0]} on:mouseover={() => setHighlightedZone(zoneDetails[0])} on:click={ev => handleTimezoneUpdate(ev, zoneLabel)}>
...
<style>
  ...
  .tz-dropdown {
    background-color: var(--accents-50, #fff);
    border: 1px solid var(--accents-100, rgba(0, 0, 0, 0.2));
    box-shadow: 0 1px 6px 0 var(--accents-100, rgba(0, 0, 0, 0.2));
    border-radius: 4px;
    display: flex;
  ...
  input {
    border: 1px solid var(--accents-400, #acacac);
    border-radius: 4px;
